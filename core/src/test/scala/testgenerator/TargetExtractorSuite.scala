...
class TargetExtractorSuite extends FunSuite with ShouldMatchers {
  ...
  test("extract package") {
    val targets = extractor.extract("testgenerator")
    targets.size should be > 0
  }

  test("extract classname") {
    val targets = extractor.extract("testgenerator.MainCommand")
    targets.size should equal(1)
  }

  test("extract path") {
    {
      val targets = extractor.extract("testgenerator/MainCommand.scala")
      targets.size should equal(1)
    }
    {
      val targets = extractor.extract("testgenerator/MainCommand")
    ...
    {
      val targets = extractor.extract("src/main/scala/testgenerator/MainCommand.scala")
      targets.size should equal(1)
    }
  }

  test("extract directory") {
    val targets = extractor.extract("src/main/scala/testgenerator")
    targets.size should be > 0
  }
