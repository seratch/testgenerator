...
class TargetExtractorSuite extends FunSuite with ShouldMatchers {
  ...
  test("extract package") {
    val targets = extractor.extract("testgen")
    targets.size should be > 0
  }

  test("extract classname") {
    val targets = extractor.extract("testgen.MainCommand")
    targets.size should equal(1)
  }

  test("extract path") {
    {
      val targets = extractor.extract("testgen/MainCommand.scala")
      targets.size should equal(1)
    }
    {
      val targets = extractor.extract("testgen/MainCommand")
    ...
    {
      val targets = extractor.extract("src/main/scala/testgen/MainCommand.scala")
      targets.size should equal(1)
    }
  }

  test("extract directory") {
    val targets = extractor.extract("src/main/scala/testgen")
    targets.size should be > 0
  }
