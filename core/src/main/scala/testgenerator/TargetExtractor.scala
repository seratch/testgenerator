...
class TargetExtractor(val config: Config) {
  ...
  private[testgenerator] def extractFromDefOnly(defOnly: String, importedList: List[String]): List[Target] = {
  ...
  private[testgenerator] def readLines(path: String, encoding: String = config.encoding): List[String] = {
    Source.fromFile(new File(path), encoding).getLines.toList
  }

  private[testgenerator] def getDefOnlyAndImportedList(lines: List[String]): (String, List[String]) = {
